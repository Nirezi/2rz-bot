...
class Report(commands.Cog):
    ...
    async def bug_report(self, ctx):
        await ctx.send("バグ等の報告ですね。各項目10分以内に送信してください")
        ...
        for koumoku in koumoku_list:
            ...
            def check(m):
                return m.author == ctx.author and m.channle == ctx.channel
            ...
            try:
                ...
                if kakunin == "yes":
                    ...
                    await log_channel.send(f"<@{self.bot.owner_id}>新しいレポートです", embed=report_em)
                elif kakunin == "no":
